...
function isValidMxnPrice(value) {
  const n = Number(value);
  return Number.isFinite(n) && n >= 9.0 && n <= 9.8;
}

function firstMxnPriceFromText(text, patterns = []) {
  const source = String(text || "");
  for (const pattern of patterns) {
    const m = source.match(pattern);
    if (m && isValidMxnPrice(m[1])) return roundMxn(Number(m[1]), 3);
  }
  return null;
}
...
function estimateMxnBuySummary(result, currentPrice = null) {
  ...
  const source = getMxnTextBlob(result);
  const explicit = firstMxnPriceFromText(source, [
    /(?:買サマリ|買いサマリ|買いサポート|買サポート)\s*(?:価格|ライン|水準)?\s*(?:は|が|:|：)?\s*([9]\.\d{2,4})/,
    /(?:buySummary|buySummaryPrice|buyLine)\s*(?:=|:|：)\s*([9]\.\d{2,4})/i,
  ]);
  if (explicit != null) return explicit;
...
function buildMxnDynamicLevels(currentPrice, buySummary) {
  const current = isValidMxnPrice(currentPrice) ? Number(currentPrice) : null;
  const buy = isValidMxnPrice(buySummary) ? Number(buySummary) : current;
  if (!isValidMxnPrice(current) || !isValidMxnPrice(buy)) return null;
  ...
  const recoveryLow = buy;
  const recoveryHigh = deepBelowBuySummary ? buy + 0.009 : buy + 0.003;
  ...
  const longSl1 = deepBelowBuySummary ? current - 0.018 : current - 0.013;
  const longSl2 = current - 0.033;
  const shortSl1 = deepBelowBuySummary ? buy + 0.009 : buy + 0.008;
  const shortSl2 = deepBelowBuySummary ? buy + 0.017 : buy + 0.028;
  ...
  return {
    ...
    longTp1: round3(longTp1),
    longTp2: round3(longTp2),
    longTp3: round3(longTp3),
    shortTp1: round3(shortTp1),
    shortTp2: round3(shortTp2),
    shortTp3: round3(shortTp3),
    longSl1: round3(longSl1),
    longSl2: round3(longSl2),
    shortSl1: round3(shortSl1),
    shortSl2: round3(shortSl2),
  };
}
...
function buildMxnRiskAlertsFromLevels(levels) {
  if (!levels) {
    return [
      ...
      "現在値と買サマリが未確認のため、価格帯は再確認が必要",
      "短期足の反発確認が出るまでは見送り",
    ];
  }
  const third = levels.deepBelowBuySummary
    ? `${levels.buySummary}付近の買サマリを大きく下回っており、深押し継続に注意`
    : levels.shallowBelowBuySummary
      ? `${levels.buySummary}付近の買サマリ下で揉み合いやすく、${levels.longSl1}割れは深押し警戒`
      : `${levels.buySummary}付近の買サマリ回復後も、維持できない場合は再下落に注意`;
  return [
    "短期RSIは未確認のため、反発確認前の成行ロングは禁止",
...
function polishMxnNarrativeTone(text, levels = null, scoreState = null) {
  if (!text) return text;
  const shortDominant = Number(scoreState?.short || 0) > Number(scoreState?.long || 0);
  let value = String(text);

  value = sanitizeDirectionWords(value)
    .replace(/方向はロング優勢だが押し目確認待ち。?/g, "日足のロング背景は残るが、短期は弱く、現在は反発確認待ち。")
    .replace(/方向はロング優勢だが押し目買い待ち。?/g, "日足のロング背景は残るが、短期は弱く、現在は反発確認待ち。")
    .replace(/ロング優勢だが押し目確認待ち/g, "ロング背景は残るが反発確認待ち")
    .replace(/ロング優勢だが押し目買い待ち/g, "ロング背景は残るが反発確認待ち")
    .replace(/1時間足・日足にはロング背景があり/g, "日足にはロング背景が残るが、1時間足は反発確認前で")
    .replace(/1時間足にはロング背景があり/g, "1時間足は反発確認前で")
    .replace(/1時間足にはロング背景が残るが/g, "1時間足は反発確認前で")
    .replace(/1時間足にロング背景はあるが/g, "1時間足は反発確認前で")
    ...
    .replace(/短期ロング加点/g, "反発確認待ち")
    .replace(/短期ロング加点材料/g, "反発確認待ち材料")
    .replace(/日足EMAは3本とも上向きで上昇基調。?/g, "日足では上昇背景が残る。")
    .replace(/日足EMAは上抜け維持/g, "日足ではEMA帯を上回っており、上位足のロング背景は残る")
    .replace(/日足MACDはまだ上向き基調でロング加点/g, "日足では上昇背景が残る")
    .replace(/4時間足はEMA割れやMACD上向き転換直後で調整局面だが、日足押し目と判断可能。?/g, "4時間足は買サマリを下回り、反発確認はまだ不足。")
    .replace(/4時間足MACDは上向き転換で押し目調整状態だが安値は更新せず/g, "4時間足は下落後の調整局面で、買サマリ回復まではロング判断を急がない")
    .replace(/4時間足はMACDが下向き傾向で下落継続し押し目弱く短期ショート寄り/g, "4時間足は買サマリを下回り、反発確認はまだ不足")
    .replace(/4時間足はMACD下向き継続中で警戒が必要/g, "4時間足は買サマリを下回り、反発確認はまだ不足")
    .replace(/4時間足は[^。]*買サマリ[^。]*反発期待[^。]*。?/g, "4時間足は買サマリを下回り、反発確認はまだ不足。")
    .replace(/4時間足は[^。]*MACD[^。]*上向き転換[^。]*。?/g, "4時間足は下落後の調整局面で、買サマリ回復まではロング判断を急がない。")
    .replace(/短期足は下落の勢いがやや鈍化し下落鈍化中/g, "短期足は下向きだが、下落の勢いはやや鈍化")
    .replace(/下落の勢いがやや鈍化し下落鈍化中/g, "下落の勢いはやや鈍化")
    .replace(/下落の勢いがやや鈍化中/g, "下落の勢いはやや鈍化")
    .replace(/成行ロングは禁止方向/g, "成行ロングは禁止。方向")
    .replace(/禁止方向/g, "禁止。方向")
    .replace(/短期RSIは未確認のため反発確認前/g, "短期RSIは未確認のため、反発確認前")
    .replace(/。\s*現在値/g, "。 現在値")
    .replace(/。\s*短期RSI/g, "。 短期RSI")
    .replace(/\s+/g, " ")
    .trim();
...
function normalizeMxnSwapResult(result) {
  ...
  ["summary", "risk", "entryTrigger", "entryPlan", "cancelCondition", "takeProfitPlan", "stopPlan"].forEach((key) => {
    if (next[key]) next[key] = polishMxnTimeframeText(polishMxnSwapText(sanitizeMxnSwapText(sanitizeDirectionWords(next[key]))));
  });
  if (Array.isArray(next.reasons)) {
    next.reasons = next.reasons.map((v) => polishMxnTimeframeText(polishMxnSwapText(sanitizeMxnSwapText(sanitizeDirectionWords(v)))));
  }
  ...
  const dynamicSummary = buildMxnSummaryFromLevels(levels, scoreState);
  if (dynamicSummary) {
    next.summary = polishMxnTimeframeText(polishMxnNarrativeTone(dynamicSummary, levels, scoreState)).trim();
  } else {
    const currentLine = "現在値と買サマリを確認し、短期足の反発を待つ場面。";
    const baseSummary = polishMxnNarrativeTone(String(next.summary || "").trim(), levels, scoreState);
    const rsiLine = /短期RSIは未確認/.test(baseSummary)
      ? ""
      : "短期RSIは未確認のため断定せず、";
    next.summary = polishMxnTimeframeText(
      `${baseSummary ? baseSummary + " " : ""}${currentLine} ${rsiLine}短期足の下げ止まり・陽線確定・EMA帯回復を待つ場面。`
    ).trim();
  }
  ...
  next.riskAlerts = buildMxnRiskAlertsFromLevels(levels);
  next.risk = next.riskAlerts.join("\n");

  next.reasons = buildMxnReasonsFromLevels(levels, scoreState)
    .map((v) => polishMxnTimeframeText(polishMxnNarrativeTone(v, levels, scoreState)));
...
function hasInvalidUsdPriceRelations(result) {
  ...
  if (longRange) {
    ...
    const longStop = getUsdLongStopPrices(result);
    const longCancel = getUsdLongCancelPrices(result);

    if (longTp.length && longTp.some((price) => price <= longRange.high)) return true;
    if (longStop.length && longStop.some((price) => price >= longRange.low)) return true;
    if (longCancel.length && longCancel.some((price) => price >= longRange.low)) return true;
  }

  if (shortRange) {
    const shortTp = getUsdShortTakeProfitPrices(result);
    const shortStop = getUsdShortStopPrices(result);
    const shortCancel = getUsdShortCancelPrices(result);

    if (shortTp.length && shortTp.some((price) => price >= shortRange.low)) return true;
    if (shortStop.length && shortStop.some((price) => price <= shortRange.high)) return true;
    if (shortCancel.length && shortCancel.some((price) => price <= shortRange.high)) return true;
  }

  return false;
}

function rebuildUsdJpyLevelsFromCurrentPrice(result, currentPrice) {
  const anchor = Number(currentPrice);
  if (!Number.isFinite(anchor) || anchor < 100 || anchor > 200) return result;

  const next = { ...result, __usdCurrentAnchor: Number(anchor.toFixed(3)) };
...
function ensureUsdConcretePriceText(result) {
  ...
  const needsConcrete =
    ...
    hasInvalidUsdPriceRelations(next);
...
function normalizeUsdJpyShortText(result) {
  ...
  const normalizeText = (text) => {
    ...
    if (diff >= 10) {
      const diffMessage = diff >= 20
        ? `LONG/SHORTの点差は${diff}点で、方向優位性はあるが強くはない`
        : `LONG/SHORTの点差は${diff}点で、方向優位性はまだ限定的`;
      value = value
        .replace(/LONG\/SHORTの点差が10点未満で方向優位性が弱い/g, diffMessage)
        .replace(/LONG\/SHORTの点差が10点未満/g, `LONG/SHORTの点差は${diff}点`)
        .replace(/LONG\/SHORTの点差は20点未満で方向優位性はやや限定的/g, diffMessage)
        .replace(/LONG\/SHORTの点差は20点未満/g, `LONG/SHORTの点差は${diff}点`)
        .replace(/点差が10点未満/g, `点差は${diff}点`)
  ...
  if (signedDiff > 0) {
    ...
    if (diff < 15) {
      ...
      next.decision = "見送り〜ショート寄り";
      next.state = "方向待ち / 反落確認待ち";
    } else if (diff < 30) {
      next.decision = "ショート寄り";
      next.state = "戻り売り待ち / 反落確認待ち";
    } else {
      next.decision = "ショート優勢";
      next.state = "戻り売り待ち / 反落確認待ち";
    }
    next.entryStatus = "WAIT";
  } else {
  ...
  // 最終表示直前にENTRY/CANCEL/TP/STOPの上下関係を検証。
  // 1つでも崩れていれば、現在値アンカーから全価格を再生成する。
